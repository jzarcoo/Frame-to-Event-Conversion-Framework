...
Each RGB frame is converted to grayscale. The processed frame is compared with the last event frame, and pixels whose intensity change exceeds a predefined threshold are marked as events.
...
### 3.4 Compatibility with Spiking Neural Networks

To evaluate whether the proposed frame-to-event conversion preserves the information required by Spiking Neural Networks (SNNs), we used the pretrained DenseNet-SNN backbone provided by the [SFOD (Spiking Fusion Object Detector)](https://github.com/yimeng-fan/SFOD) project. The network was trained exclusively on the [N-CARS event dataset](https://www.prophesee.ai/2018/03/13/dataset-n-cars/) and kept fixed during all experiments. Rather than evaluating cross-dataset classification performance, the network was used as a feature probe to compare its response when processing real DSEC event streams and synthetic event streams generated by our framework.

| Input | Accuracy | Cars | Background |
|-------|---------:|------:|-----------:|
| Real DSEC Events | 10.16% | 10.43% | 0.83% |
| Synthetic Events | 12.84% | 10.40% | 99.16% |

The following figure compares the row-normalized confusion matrices obtained using the pretrained DenseNet-SNN when processing real DSEC events and synthetic events generated by our framework.

![Confusion Matrix Comparison](./figures/confusion_matrix_comparison.png)

Although the classifier does not generalize to DSEC because of the substantial domain shift between N-CARS and DSEC, both inputs produce nearly identical recognition performance for the *Cars* class (10.43% versus 10.40%). This indicates that the synthetic event stream preserves the discriminative structures required by the SNN despite being generated from conventional RGB frames.

Interestingly, the synthetic events also produce significantly fewer false activations on background samples, resulting in a higher overall accuracy while preserving essentially identical vehicle recognition performance. These results suggest that the proposed conversion framework generates event representations that remain compatible with feature representations learned from real event-camera data.
...
In the context of Spiking Neural Networks, the compatibility experiment demonstrates that this increased sparsity does not significantly alter the discriminative information required for vehicle recognition. Despite producing only 8.83% of the events recorded by the physical sensor, the generated event streams elicit nearly identical responses from a pretrained SNN, suggesting that the proposed conversion preserves the most relevant motion and edge information while discarding a substantial amount of redundant activity.
